...
class Xoshiro256PlusPlusGenerator
  implements GeneratorInterface<Xoshiro256PlusPlusGeneratorState>
{
  s0: bigint;
  s1: bigint;
  s2: bigint;
  ...
  state(): Xoshiro256PlusPlusGeneratorState {
    return {
      s0: this.s0 & MASK_64,
      s1: this.s1 & MASK_64,
      s2: this.s2 & MASK_64,
  ...
  setState(state: Xoshiro256PlusPlusGeneratorState): void {
...
export const xoshiro256plusplus: PRNGAlgorithm<
  Xoshiro256PlusPlusGeneratorState
> = (seed, state) => {
  const generator = new Xoshiro256PlusPlusGenerator(seed);
