import type { GeneratorInterface, PRNGAlgorithm, Xoshiro128PlusPlusGeneratorState } from 'src/types';
import { rotl } from 'src/utils';

class Xoshiro128PlusPlusGenerator implements GeneratorInterface<Xoshiro128PlusPlusGeneratorState> {
  s0: number;
  s1: number;
  s2: number;
  s3: number;
  ...
  next(): number {
    const result = rotl((this.s1 * 5) >>> 0, 7) * 9;
...
export const xoshiro128plusplus: PRNGAlgorithm<Xoshiro128PlusPlusGeneratorState> = (seed, state) => {
